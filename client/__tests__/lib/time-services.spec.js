...
describe('Time Services input validation', () => {
  it('should throw an error when parameter objects are incomplete', () => {
    let getMinutesSinceMidnight = function() {
      TimeServices.getMinutesSinceMidnight({hour: 10})
    }
    let getDaysSinceNewYear = function() {
  ...
  it('should throw an error when parameters are absent', () => {
    let convertFromMinutesAfterMidnight = function() {
      TimeServices.convertFromMinutesAfterMidnight()
    }
    let convertFromDaysAfterNewYear = function() {
      TimeServices.convertFromDaysAfterNewYear()
    }
    let getMinutesSinceMidnight = function() {
    ...
    expect(convertFromDaysAfterNewYear).toThrow()
    expect(getMinutesSinceMidnight).toThrow()
    expect(getDaysSinceNewYear).toThrow()
    expect(formattedTime).toThrow()
    expect(formattedDate).toThrow()
