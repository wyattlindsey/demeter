...
internals.Date.prototype._base = function (value, state, options) {
    ...
    if (result.value instanceof Date && !isNaN(result.value.getTime())) {
        result.errors = null;} else 

    {
        var type = /*istanbul ignore next*/void 0;
        if (internals.isIsoDate(this._flags.format)) {
            type = 'isoDate';} else 

        if (this._flags.timestamp) {
            type = 'timestamp.' + this._flags.timestamp;} else 
...
internals.toDate = function (value, format, timestamp, multiplier) {
    ...
    typeof value === 'number' && !isNaN(value) && isFinite(value)) {
        ...
        var date = /*istanbul ignore next*/void 0;
        if (format) {
            if (internals.isIsoDate(format)) {
                date = format.test(value) ? new Date(value) : internals.invalidDate;} else 

            {
                date = Moment(value, format, true);
                date = date.isValid() ? date.toDate() : internals.invalidDate;}} else 


        if (timestamp && multiplier) {
            date = new Date(value * multiplier);} else 
...
internals.compare = function (type, compare) {
    ...
    return function (date) {/*istanbul ignore next*/var _this = this;
        ...
        return this._test(type, date, function (value, state, options) {
            ...
            if (isNow) {
                compareTo = Date.now();} else 

            if (isRef) {
                compareTo = internals.toDate(date(state.parent, options));

                if (!compareTo) {
                    return (/*istanbul ignore next*/_this.createError('date.ref', { ref: date.key }, state, options));}


                compareTo = compareTo.getTime();} else 
            ...
            return (/*istanbul ignore next*/_this.createError('date.' + type, { limit: new Date(compareTo) }, state, options));});};};




internals.Date.prototype.min = internals.compare('min', function (value, date) /*istanbul ignore next*/{return value >= date;});
internals.Date.prototype.max = internals.compare('max', function (value, date) /*istanbul ignore next*/{return value <= date;});

internals.Date.prototype.format = function (format) {

    Hoek.assert(typeof format === 'string' || Array.isArray(format) && format.every(function (f) /*istanbul ignore next*/{return typeof f === 'string';}), 'Invalid format.');
