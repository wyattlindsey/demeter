...
internals.checkIpV6 = function (items) {
    ...
    return items.every(function (value) /*istanbul ignore next*/{return internals.regex.ipV6.test(value);});};
...
exports.validate = internals.validate = function (email, options, callback) {
    ...
    var diagnose = /*istanbul ignore next*/void 0;
    var threshold = /*istanbul ignore next*/void 0;

    if (typeof options.errorLevel === 'number') {
        diagnose = true;
        threshold = options.errorLevel;} else 
    ...
    if (options.tldWhitelist) {
        if (typeof options.tldWhitelist === 'string') {
            options.tldWhitelist = [options.tldWhitelist];} else 

        if ( /*istanbul ignore next*/_typeof(options.tldWhitelist) !== 'object') {
    ...
    if (options.tldBlacklist) {
        if (typeof options.tldBlacklist === 'string') {
            options.tldBlacklist = [options.tldBlacklist];} else 

        if ( /*istanbul ignore next*/_typeof(options.tldBlacklist) !== 'object') {
    ...
    var elementLength = 0;
    var crlfCount = 0;
    var charCode = /*istanbul ignore next*/void 0;
    ...
    var token = /*istanbul ignore next*/void 0; // Token is used outside the loop, must declare similarly
    for (var i = 0; i < emailLength; ++i) {
        ...
        switch (context.now) {
                ...
                switch (token) {
                    ...
                    case '(':
                        if (elementLength === 0) {
                            ...
                            updateResult(elementCount === 0 ? internals.diagnoses.cfwsComment : internals.diagnoses.deprecatedComment);} else 
                    ...
                    case '.':
                        if (elementLength === 0) {
                            // Another dot, already?
                            updateResult(elementCount === 0 ? internals.diagnoses.errDotStart : internals.diagnoses.errConsecutiveDots);} else 
                    ...
                    case '"':
                        if (elementLength === 0) {
                            ...
                            assertEnd = true;
                            context.stack.push(context.now);
                            context.now = internals.components.contextQuotedString;} else 
                    ...
                    case '\t':
                        if (elementLength === 0) {
                            updateResult(elementCount === 0 ? internals.diagnoses.cfwsFWS : internals.diagnoses.deprecatedFWS);} else 
                    ...
                    case '@':
                        ...
                        if (parseData.local.length === 0) {
                            // Fatal error
                            updateResult(internals.diagnoses.errNoLocalPart);} else 
                ...
                switch (token) {
                    ...
                    case '(':
                        if (elementLength === 0) {
                            ...
                            // http://tools.ietf.org/html/rfc5322#section-3.4.1
                            updateResult(elementCount === 0 ? internals.diagnoses.deprecatedCFWSNearAt : internals.diagnoses.deprecatedComment);} else 
                    ...
                    case '.':
                        ...
                        if (hyphenFlag) {
                            ...
                            updateResult(internals.diagnoses.errDomainHyphenEnd);} else 
                    ...
                    case '\t':
                        if (elementLength === 0) {
                            updateResult(elementCount === 0 ? internals.diagnoses.deprecatedCFWSNearAt : internals.diagnoses.deprecatedFWS);} else 
                    ...
                    default:
                        ...
                        if (charCode < 33 || charCode > 126 || internals.specials(charCode)) {
                            // Fatal error
                            updateResult(internals.diagnoses.errExpectingATEXT);} else 
                    ...
                    case ']':
                        if (maxResult < internals.categories.deprecated) {
                            ...
                            if (index === 0) {
                                ...
                                updateResult(internals.diagnoses.rfc5321AddressLiteral);} else 

                            if (addressLiteral.slice(0, 5).toLowerCase() !== 'ipv6:') {
                                updateResult(internals.diagnoses.rfc5322DomainLiteral);} else 

                            {
                                ...
                                if (! ~index) {
                                    ...
                                    if (groups.length !== maxGroups) {
                                        updateResult(internals.diagnoses.rfc5322IPv6GroupCount);}} else 


                                if (index !== match.lastIndexOf('::')) {
                                    updateResult(internals.diagnoses.rfc5322IPv62x2xColon);} else 

                                {
                                    ...
                                    if (groups.length > maxGroups) {
                                        updateResult(internals.diagnoses.rfc5322IPv6MaxGroups);} else 
                                ...
                                if (match[0] === ':' && match[1] !== ':') {
                                    updateResult(internals.diagnoses.rfc5322IPv6ColonStart);} else 

                                if (match[match.length - 1] === ':' && match[match.length - 2] !== ':') {
                                    updateResult(internals.diagnoses.rfc5322IPv6ColonEnd);} else 

                                if (internals.checkIpV6(groups)) {
                                    updateResult(internals.diagnoses.rfc5321AddressLiteral);} else 

                                {
                                    updateResult(internals.diagnoses.rfc5322IPv6BadCharacter);}}} else 
                ...
                switch (token) {
                    ...
                    default:
                        ...
                        if (charCode > 127 || charCode === 0 || charCode === 10) {
                            updateResult(internals.diagnoses.errExpectingQTEXT);} else 
            ...
            case internals.components.contextQuotedPair:
                ...
                if (charCode > 127) {
                    // Fatal error
                    updateResult(internals.diagnoses.errExpectingQPair);} else 

                if (charCode < 31 && charCode !== 9 || charCode === 127) {
                    // ' ' and '\t' are allowed
            ...
            case internals.components.contextFWS:
                ...
                if (prevToken === '\r') {
                    ...
                    if (++crlfCount > 1) {
                        ...
                        updateResult(internals.diagnoses.deprecatedFWS);} else 
    ...
    if (maxResult < internals.categories.rfc5322) {
        ...
        if (context.now === internals.components.contextQuotedString) {
            updateResult(internals.diagnoses.errUnclosedQuotedString);} else 

        if (context.now === internals.components.contextQuotedPair) {
            updateResult(internals.diagnoses.errBackslashEnd);} else 

        if (context.now === internals.components.contextComment) {
            updateResult(internals.diagnoses.errUnclosedComment);} else 

        if (context.now === internals.components.literal) {
            updateResult(internals.diagnoses.errUnclosedDomainLiteral);} else 

        if (token === '\r') {
            updateResult(internals.diagnoses.errFWSCRLFEnd);} else 

        if (parseData.domain.length === 0) {
            updateResult(internals.diagnoses.errNoDomain);} else 

        if (elementLength === 0) {
            updateResult(internals.diagnoses.errDotEnd);} else 
        ...
        // Other errors
                ...
                updateResult(internals.diagnoses.rfc5322DomainTooLong);} else 

            if (parseData.local.length + parseData.domain.length + /* '@' */1 > 254) {
                // http://tools.ietf.org/html/rfc5321#section-4.1.2
                //   Forward-path   = Path
                //
                //   Path           = "<" [ A-d-l ":" ] Mailbox ">"
                ...
                //   to be 254.
                updateResult(internals.diagnoses.rfc5322TooLong);} else 

            if (elementLength > 63) {
                // http://tools.ietf.org/html/rfc1035#section-2.3.4
                // labels   63 octets or less
                updateResult(internals.diagnoses.rfc5322LabelTooLong);} else 

            if (options.minDomainAtoms && atomData.domains.length < options.minDomainAtoms) {
                updateResult(internals.diagnoses.errDomainTooShort);} else 

            if (options.tldWhitelist || options.tldBlacklist) {
    ...
    var finish = function finish() {
        ...
        if (!dnsPositive && maxResult < internals.categories.dnsWarn) {
            ...
            if (code <= 57) {
                updateResult(internals.diagnoses.rfc5321TLDNumeric);} else 
    ...
    if (options.checkDNS && maxResult < internals.categories.dnsWarn) {/*istanbul ignore next*/(function () {
