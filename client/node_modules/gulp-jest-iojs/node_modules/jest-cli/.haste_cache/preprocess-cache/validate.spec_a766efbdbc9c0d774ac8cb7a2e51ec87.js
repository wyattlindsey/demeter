...
describe('validate rejection', function () {
  ...
  it('throws error on invalid date', function () {
    expect(function () {Validate.date({ month: 13, date: 1, year: 2000 });}).toThrow('argument out of range');
    expect(function () {Validate.date({ month: 11, date: 32, year: 2000 });}).toThrow('argument out of range');
    expect(function () {Validate.date({ month: 11, date: 1, year: 10000 });}).toThrow('argument out of range');

    expect(function () {Validate.date({ month: 11, date: 1 });}).toThrow('missing arguments');
    expect(function () {Validate.date({ year: 1996 });}).toThrow('missing arguments');});


  it('throws error on invalid time', function () {
    expect(function () {Validate.time({ hour: 25, minute: 1 });}).toThrow('argument out of range');
    expect(function () {Validate.time({ hour: 12, minute: 62 });}).toThrow('argument out of range');
  ...
  it('throws error when argument out of range', function () {
    expect(function () {Validate.isWithinRange(100, 0, 50);}).toThrow('arument out of range');
    expect(function () {Validate.isWithinRange(-50, 0, 50);}).toThrow('arument out of range');
