...
var utils = { 
  ...
  getContainerDimensions: function getContainerDimensions(containerNode) {
    ...
    if (containerNode.tagName === 'BODY') {
      ...
      scroll = _domHelpersQueryScrollTop2['default'](_ownerDocument2['default'](containerNode).documentElement) || _domHelpersQueryScrollTop2['default'](containerNode);} else 
    {
  ...
  calcOverlayPosition: function calcOverlayPosition(placement, overlayNode, target, container, padding) {
    ...
    if (placement === 'left' || placement === 'right') {
      ...
      if (placement === 'left') {
        positionLeft = childOffset.left - overlayWidth;} else 
      {
      ...
      arrowOffsetLeft = void 0;} else 
    if (placement === 'top' || placement === 'bottom') {
      positionLeft = childOffset.left + (childOffset.width - overlayWidth) / 2;

      if (placement === 'top') {
        positionTop = childOffset.top - overlayHeight;} else 
      {
...
function getTopDelta(top, overlayHeight, container, padding) {
  ...
  if (topEdgeOffset < 0) {
    return -topEdgeOffset;} else 
  if (bottomEdgeOffset > containerHeight) {
    return containerHeight - bottomEdgeOffset;} else 
  {
    return 0;}}
...
function getLeftDelta(left, overlayWidth, container, padding) {
  ...
  if (leftEdgeOffset < 0) {
    return -leftEdgeOffset;} else 
  if (rightEdgeOffset > containerWidth) {
    return containerWidth - rightEdgeOffset;} else 
  {
    return 0;}}
